...
* Ambiente de execucao das estatisticas
 ...
 R_SESSION_TMPDIR      /tmp/RtmpnbyzuP
...
#+NAME:  plot_boxplot(myDados$Esp_Pre, "Estresse Percebido - Pré" BY Presenncial ou Remoto)
#+begin_src R :session s1 :file boxplot_Esp_Pre_BY_PresOuRemoto.png :results output graphics file  :exports both
    plot_boxplot(
      var_y = myDados$Esp_Pre,
      var_x = myDados$Rem_Pres,
      strTitle = "Estresse Percebido - Pre",
      strSubTitle = "Agrupado por Participacao: Presencial ou Remota",
      strVarYLabel = "Estress Percebido - Pre",
      strVarXLabel = "Participacao: Presencial ou Remota")
#+end_src

#+RESULTS: plot_boxplot(myDados$Esp_Pre, "Estresse Percebido - Pré" BY Presenncial ou Remoto)
[[file:boxplot_Esp_Pre_BY_PresOuRemoto.png]]
...
#+NAME:  plot_boxplot(myDados$Esp_Pos, "Estresse Percebido - Pos" BY Participacao Presencial ou Remoto
#+begin_src R :session s1 :file boxplot_Esp_Pos_BY_ParticipacaoPresencialOuRemota.png :results output graphics file  :exports both
    plot_boxplot(
      var_y = myDados$ESP_Pos,
      var_x = myDados$Rem_Pres,
      strTitle = "Estresse Percebido - Pos",
...
#+RESULTS: plot_boxplot(myDados$Esp_Pos, "Estresse Percebido - Pos" BY Participacao Presencial ou Remoto
[[file:boxplot_Esp_Pos_BY_ParticipacaoPresencialOuRemota.png]]
...
#+NAME:  plot_boxplot - myDados$FPP.D.pre BY Participacao Presencial ou Remota
#+begin_src R :session s1 :file boxplot_FPP.D.pre_BY_ParticipacaoPresencialOuRemota.png :results output graphics file  :exports both
    plot_boxplot(
      var_y = myDados$FPP.D.pre,
      var_x = myDados$Rem_Pres,
      strTitle = "FPP.D.pre",
...
#+RESULTS: plot_boxplot - myDados$FPP.D.pre BY Participacao Presencial ou Remota
[[file:boxplot_FPP.D.pre_BY_ParticipacaoPresencialOuRemota.png]]
...
#+begin_src R :session s1 :results output :exports both
  print("=== Dados Com missing...")
  Alc.Func_Pre_e_Pos <- cbind(myDados$Alc.Func.pre,myDados$Alc.Func.pos)
  Alc.Func_Pre_e_Pos
...
alternative hypothesis: true location shift is not equal to 0
Warning messages:
1: In wilcox.test.default(Alc.Func_Pre_e_Pos_SemMissing[, 1], Alc.Func_Pre_e_Pos_SemMissing[,  :
  nÃ£o Ã© possÃ­vel computar o valor de p exato com o de desempate
2: In wilcox.test.default(Alc.Func_Pre_e_Pos_SemMissing[, 1], Alc.Func_Pre_e_Pos_SemMissing[,  :
  cannot compute exact p-value with zeroes
